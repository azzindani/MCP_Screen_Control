...
class TestStartTask:
    ...
    def test_snapshot_created(self):
        ...
        with patch("engine.decompose_prompt", side_effect=self._mock_decompose):
            import engine
            engine.start_task("test task")
        # Either objective.md now exists (backup created on second call) or no backup needed on first
        # Just verify workspace was created
        assert TMP.exists()
...
class TestFindElement:
    ...
    def test_high_confidence_returns_coords(self):
        ...
        high_conf = json.dumps({"x": 100, "y": 50, "confidence": 0.9, "description": "OK button"})
        with patch("_sc_vision._llm_call", return_value=high_conf):
            import engine
    ...
    def test_low_confidence_triggers_retry(self):
        ...
        with patch("_sc_vision._llm_call", side_effect=fake_llm):
            import engine
            result = engine.find_element_tool("click the OK button")
    ...
    def test_second_failure_sets_needs_clarification(self):
        ...
        with patch("_sc_vision._llm_call", return_value=low_conf):
            import engine
...
class TestVerifyStep:
    ...
    def test_success_marks_step_done(self):
        ...
        with patch("_sc_vision._llm_call", return_value=success_resp), \
             patch("_sc_capture.mss") as mock_mss:
            mock_mss.return_value.__enter__.return_value.monitors = [{"left": 0, "top": 0, "width": 800, "height": 480}]
            mock_mss.return_value.__enter__.return_value.grab.return_value.rgb = b"\xff" * (800 * 480 * 3)
            mock_mss.return_value.__enter__.return_value.grab.return_value.size = (800, 480)
            import engine
            # Use the already-captured image directly
            with patch("_sc_verify.capture_screen", return_value={"success": True, "path": str(TMP / "screen_current.png"), "width": 800, "height": 480}):
    ...
    def test_three_failures_set_blocked(self):
        ...
        with patch("_sc_vision._llm_call", return_value=fail_resp), \
             patch("_sc_verify.capture_screen", return_value={"success": True, "path": str(TMP / "screen_current.png"), "width": 800, "height": 480}):
            import engine
            for _ in range(3):
                result = engine.verify_step_tool("click OK")
